	...
	try {
		...
		//#region Skip File Output if outfile exists, is based on the same codebase (determined by hash) and has used the same compier-version
			...
			if (fs.existsSync(outFilePath)) {
				...
				try{
					if(!options.force){
						const firstLineData = fs.readFileSync(outFilePath, "utf-8").split('\n')[0];
		...
		try{
			...
			if(!twineCodeRaw){
				...
				twineCode = `//qsrc2twResult={"version":"${thisFilesHash}","code":"${codeHash}","time":"${(new Date().toISOString())}"}`+"\n"+twineCodeRaw;
				fs.writeFileSync(outFilePath, twineCode);
			}
